...
if [[ "$(uname)" == MINGW* ]]; then
  ...
  source $RHUMBA_PATH/Scripts/activate

  Rscript -e "rhumba::set_config('auto_yes', 'true'); rhumba::install('xtensor')"

  test -d $RHUMBA_PATH/Library/include/xtensor
  test -f $RHUMBA_PATH/Library/include/xtensor/xtensor.hpp
else
  bash $INSTALLER_PATH -b -p $RHUMBA_PATH
  source $RHUMBA_PATH/bin/activate

  Rscript -e "rhumba::set_config('auto_yes', 'true'); rhumba::install('xtensor')"

  test -d $RHUMBA_PATH/include/xtensor
  test -f $RHUMBA_PATH/include/xtensor/xtensor.hpp
fi
